...
export default function Credits() {
  ...
  // usamos el store tal cual; si añadiste setCounts/addPaid se aprovechan, si no no rompe
  ...
  // sincroniza al entrar (si no hay n8n -> mock)
  useEffect(() => {
    (async () => {
      try {
        ...
        // si falla, mantenemos los valores locales
      }
    })();
  ...
  async function handleBuy(packCredits: number, label: string) {
    ...
    try {
      ...
      creditStore?.setCounts?.(c.freeLeft, c.paidLeft);
      creditStore?.addPaid?.(packCredits); // respaldo local opcional
              ...
              >
                ...
                <ul className="text-sm text-gray-700 space-y-2 flex-1">
                  <li className="flex items-center gap-2">
                    <Check size={16} /> +{p.credits} credits
                  </li>
                  {p.features.map((f) => (
